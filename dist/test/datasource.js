...
exports.AmplitudeDatasource = undefined;
...
var AmplitudeDatasource = exports.AmplitudeDatasource = function () {
  ...
  _createClass(AmplitudeDatasource, [{
    ...
    value: function metricFindProps(eventName) {
      ...
      this.fetching_props[eventName] = this.backendSrv.datasourceRequest({
        ...
        self._cached_tags = _lodash2.default.map(response.data.data, function (event) {
          return {
            text: event.name,
            value: event.name
          };
    ...
    value: function getTarget(from, to, target) {
      ...
      return this.backendSrv.datasourceRequest({
        ...
        if (target.uniquePropCount) {
          points = [];
          _lodash2.default.each(response.data.data.series, function (row, i) {
            _lodash2.default.each(row, function (val, x) {
              if (!points[x]) {
                points[x] = 0;
              }
              if (val) {
        ...
        var dates = _lodash2.default.map(response.data.data.xValues, function (dateString) {
          return new Date(dateString).getTime();
        });

        return {
          "target": target.alias || target.refId,
          "datapoints": _lodash2.default.zip(points, dates)
        };
      });
    }
  }, {
    ...
    value: function query(options) {
      ...
      var self = this;
      return this.q.all(_lodash2.default.map(targets, function (target) {
